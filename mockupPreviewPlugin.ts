...
export const LOVABLE_CANVAS_MOCKUP_PREVIEW_RUNTIME_FINGERPRINT =
  "sha256:1527516845045afe6120f3a0363def4361e79db4b635e85b8ec719952a691f1e";
  ...
  async function writePreviewRoutes(): Promise<boolean> {
    ...
    const routeSource = (source: string) =>
      hasSitemapInventory
        ...
        : source;
    const mockupChanged = await writeFileIfChanged(
      path.join(root, MOCKUP_ROUTE_FILE),
      routeSource(MOCKUP_ROUTE_SOURCE),
    );
    const componentChanged = await writeFileIfChanged(
      path.join(root, COMPONENT_ROUTE_FILE),
      routeSource(COMPONENT_ROUTE_SOURCE),
    );
